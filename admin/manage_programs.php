...
if (isset($_POST['add_program'])) {
    ...
    if (!isset($error) && !empty($class) && ($category_id || $category_name) && !empty($description)) {
        ...
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->bind_param("ssidsss", $class, $category_name_for_insert, $final_category_id, $price, $discount_price, $imagePath, $description);
            if ($stmt->execute()) {
...
if (isset($_POST['edit_program'])) {
    ...
    if (!isset($error) && !empty($class) && ($category_id || $category_name) && !empty($description)) {
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssidsssi", $class, $category_name_for_update, $final_category_id, $price, $discount_price, $imagePath, $description, $id);
        if ($stmt->execute()) {
